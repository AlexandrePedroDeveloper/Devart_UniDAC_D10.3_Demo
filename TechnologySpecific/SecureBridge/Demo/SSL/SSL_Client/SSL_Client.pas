...
type
  TSSLClientFrame = class(TDemoFrame)
    ...
    edCACertName: TEdit;
    edKeyName: TEdit;
    cbTrustServerSertificate: TCheckBox;
...
begin
  ...
  try
    ...
    with Registry do begin
      ...
      WriteBool('Trust Server Sertificate', cbTrustServerSertificate.Checked);
  ...
  try
    with Registry do begin
      if OpenKey(RegistryKey, False) then begin
        ...
        if ValueExists('Trust Server Sertificate') then
          cbTrustServerSertificate.Checked := ReadBool('Trust Server Sertificate');
...
begin
  ...
  if UniConnection.ProviderName <> 'Oracle' then
    ...
    CRSSLIOHandler.SecurityOptions.TrustServerCertificate := cbTrustServerSertificate.Checked;
