...
{$ELSE}
  ...
  Devart.UniDac.DBF.DBFUniProvider
{$ENDIF}
  ;
...
type
  TUniDACForm = class(TDemoForm)
    ...
    scCreate_DBF: TUniScript;
    scDrop_DBF: TUniScript;
...
{$IFNDEF STD}
  Dump, Loader,
{$ENDIF}
  CachedUpdates, ConnectDialog, FilterAndIndex, MasterDetail,
...
begin
  ...
  if UniConnection.ProviderName = 'DBF' then
    scCreate_DBF.Execute
end;
